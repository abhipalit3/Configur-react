...
export class MeasurementTool {
  ...
  updatePreviewLabel(p1, p2, event) {
    ...
    if (!this.previewLabel) {
      ...
      this.previewLabel.className = 'hypar-measurement-preview-label'
  ...
  createMeasurementLabel(p1, p2, distance, measurementId) {
    ...
    label.className = 'hypar-measurement-label'
    label.dataset.measurementId = measurementId
    
    // Hypar-style label content with feet and inches
    label.innerHTML = `
      <div class="hypar-measurement-value">${formattedDistance}</div>
    `
  ...
  updateMeasurementHighlight(measurementId, isSelected) {
    ...
    if (label && label.element) {
      if (isSelected) {
        ...
        const valueElement = label.element.querySelector('.hypar-measurement-value')
        if (valueElement) {
